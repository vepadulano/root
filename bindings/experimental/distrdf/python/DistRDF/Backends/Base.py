...
class BaseBackend(ABC):
    ...
    def execute(self, generator):
        ...
        # Check if the ranges must be built on the client or on the workers
        open_files_locally = ROOT.RDF.Experimental.Distributed.open_files_locally
        if open_files_locally:
            ranges = headnode.build_clustered_ranges()
            build_rdf_from_range = headnode.generate_rdf_creator_fromclusteredranges()
        else:
            ranges = headnode.build_percentage_ranges()
            build_rdf_from_range = headnode.generate_rdf_creator_frompercentageranges()
