...
class EmptySourceRanges(unittest.TestCase):
    ...
    def test_nentries_multipleOf_npartitions(self):
        ...
        # First case
        rng = Ranges.get_balanced_ranges(nentries_small, npartitions_small, rdf_uuid=None)
        ranges_small = emptysourceranges_to_tuples(rng)

        # Second case
        rng = Ranges.get_balanced_ranges(nentries_large, npartitions_large, rdf_uuid=None)
    ...
    def test_nentries_not_multipleOf_npartitions(self):
        ...
        rng = Ranges.get_balanced_ranges(nentries_1, npartitions, rdf_uuid=None)
        ranges_1 = emptysourceranges_to_tuples(rng)

        # Example in which fractional part of
        # (nentries/npartitions) < 0.5
        rng = Ranges.get_balanced_ranges(nentries_2, npartitions, rdf_uuid=None)
    ...
    def test_nentries_greater_than_npartitions(self):
        ...
        rng = Ranges.get_balanced_ranges(nentries, npartitions, rdf_uuid=None)
...
class TreeRanges(unittest.TestCase):
    ...
    def test_clustered_ranges_with_one_cluster(self):
        ...
        percranges = Ranges.get_percentage_ranges(treenames, filenames, npartitions, friendinfo=None, rdf_uuid=None)
    ...
    def test_rdataframe_with_treename_and_filename_with_globbing(self):
        ...
        percranges = Ranges.get_percentage_ranges([treename], extracted_inputfiles, npartitions, friendinfo=None, rdf_uuid=None)
        clusteredranges = [Ranges.get_clustered_range_from_percs(percrange)[0] for percrange in percranges]
        ranges = treeranges_to_tuples(clusteredranges)
    ...
    def test_rdataframe_with_notreename_and_chain_with_subtrees(self):
        ...
        percranges = Ranges.get_percentage_ranges(
            extracted_subtreenames, extracted_filenames, npartitions, friendinfo=None, rdf_uuid=None)
        clusteredranges = [Ranges.get_clustered_range_from_percs(percrange)[0] for percrange in percranges]
        ranges = treeranges_to_tuples(clusteredranges)
